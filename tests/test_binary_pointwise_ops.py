...
def test_accuracy_add(shape, alpha, dtype):
    inp1 = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
...
def test_accuracy_bitwiseand_(shape, dtype):
    if dtype in BOOL_TYPES:
        inp1 = torch.randint(0, 2, size=shape, dtype=dtype).to(device=flag_gems.device)
        inp2 = torch.randint(0, 2, size=shape, dtype=dtype).to(device=flag_gems.device)
    else:
        inp1 = torch.randint(
...
def test_accuracy_clamp_tensor(shape, isnone, dtype):
    inp = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
    maxi = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
    mini = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
    if isnone == "min":
        mini = None
    elif isnone == "max":
        maxi = None
...
def test_accuracy_trunc_div(shape, dtype):
    ...
    if flag_gems.vendor_name == "tsingmicro":   #avoid dividing by 0
        inp2_ = torch.randn(shape, dtype=dtype, device="cpu")
        inp2_[inp2_ == 0] += 0.001
...
def test_accuracy_eq(shape, dtype):
    inp1 = torch.randint(0, 10, shape, dtype=dtype).to(device=flag_gems.device)
    inp2 = torch.randint(0, 10, shape, dtype=dtype).to(device=flag_gems.device)
    ref_inp1 = to_reference(inp1)
    ref_inp2 = to_reference(inp2)
...
def test_accuracy_gelu_and_mul(shape, approximate, dtype):
    inp1 = torch.randn(shape, dtype=dtype, device="cpu", requires_grad=True).to(flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype, device="cpu", requires_grad=True).to(flag_gems.device)
    ref_inp1 = to_reference(inp1, True)
    ref_inp2 = to_reference(inp2, True)
    ...
    out_grad = torch.randn_like(res_out.cpu()).to(flag_gems.device)
...
@pytest.mark.skipif(flag_gems.vendor_name != "tsingmicro", reason="no triton kernel")
@pytest.mark.rsub
@pytest.mark.parametrize("shape", POINTWISE_SHAPES)
@pytest.mark.parametrize("alpha", SCALARS)
@pytest.mark.parametrize("dtype", FLOAT_DTYPES)
...
def test_accuracy_where_self_out_cross_device(shape, dtype):
    inp1 = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
    inp2 = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
    cond = torch.randint(0, 2, shape, dtype=torch.bool).to(device=flag_gems.device)
...
def test_accuracy_where_self_out(shape, dtype):
    ...
    inp2 = torch.randn(shape, dtype=dtype).to(device=flag_gems.device)
    cond = torch.randint(0, 2, shape, dtype=torch.bool).to(device=flag_gems.device)
    out = torch.empty(shape, dtype=dtype).to(device=flag_gems.device)
...
def test_accuracy_polar(shape, dtype):
    abs = (torch.rand(shape, dtype=dtype, device="cpu") * 5).to(flag_gems.device)  
    angle = ((torch.rand(shape, dtype=dtype, device="cpu") - 0.5) * (
        8 * math.pi
    )).to(flag_gems.device)
    ...
    with flag_gems.use_gems(): 
        res_out_real = res_out.real.contiguous()
        res_out_imag = res_out.imag.contiguous()
    gems_assert_close(res_out_real, ref_out.real, dtype)
    gems_assert_close(res_out_imag, ref_out.imag, dtype)
...
def test_accuracy_masked_fill(shape, dtype, threshold, value):
    inp = torch.zeros(shape, dtype=dtype).to(device=flag_gems.device)
    mask = torch.randn(shape, dtype=dtype) < threshold
    mask = mask.to(device=flag_gems.device)
