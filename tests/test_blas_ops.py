...
def test_accuracy_bmm(M, N, K, dtype):
    batch = 4
    mat1 = torch.randn((batch, M, K), dtype=dtype).to(device=flag_gems.device)
    mat2 = torch.randn((batch, K, N), dtype=dtype).to(device=flag_gems.device)
    ref_mat1 = to_reference(mat1, True)
    ref_mat2 = to_reference(mat2, True)
...
def test_accuracy_mv(M, N, dtype):
    matrix = torch.randn((N, M), dtype=dtype).to(device=flag_gems.device)
    vector = torch.randn((M,), dtype=dtype).to(device=flag_gems.device)
...
def test_accuracy_outer(M, N, dtype):
    inp1 = torch.randn(M, dtype=dtype, requires_grad=True).to(device=flag_gems.device)
    inp2 = torch.randn(N, dtype=dtype, requires_grad=True).to(device=flag_gems.device)
