...
def test_accuracy_all_without_dim(shape, dtype, kind):
    if kind == "allTrue":
        inp = torch.ones(shape, dtype=dtype).to(device=flag_gems.device)
    else:
...
def test_accuracy_allclose(shape, dtype, equal_nan, gen_nan):
    ...
    if dtype in ALL_FLOAT_DTYPES:
        ...
        if gen_nan:
            ...
            inp1_.view(-1)[0] = (
                (-nan_num.cpu()) if gen_nan == 3 else nan_num
            )
            inp2_.view(-1)[0] = (
                (-nan_num.cpu()) if gen_nan >= 3 else nan_num
            )
...
def test_accuracy_max_without_dim_all_neg_inf(shape, dtype):
    inp = torch.full(
        shape, fill_value=float("-inf"), dtype=dtype).to(device=flag_gems.device
    )
    ref_inp = to_reference(inp)
