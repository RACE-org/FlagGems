...
def make_input(
    ...
    q = torch.empty(q_shape, dtype=dtype, device="cpu").uniform_(-0.05, 0.05).to(device)
    k = torch.empty(kv_shape, dtype=dtype, device="cpu").uniform_(-0.05, 0.05).to(device)
    v = torch.empty(kv_shape, dtype=dtype, device="cpu").uniform_(-0.05, 0.05).to(device)
...
def attention_ref(
    ...
    if flag_gems.vendor_name == "tsingmicro":
        # check disable swa
        if window_size[0] >= k.shape[-3]:
            window_size = (-1, window_size[1])
        if window_size[1] >= k.shape[-3]:
    ...
    if flag_gems.vendor_name == "tsingmicro":
        ...
        lse = torch.where(lse == float('-inf'), float("inf"), lse)
    ...
    if flag_gems.vendor_name == "tsingmicro":
        return output.to(dtype=dtype_og), lse
    else:
...
def test_flash_fwd_nonsquare_qk(
    ...
    if flag_gems.vendor_name == "tsingmicro":
        torch_out, torch_lse = attention_ref(ref_q, ref_k, ref_v, scale, causal=is_causal)
    else:
...
def test_flash_fwd_gqa_alibi_softcap(
    ...
    ref_attn_bias = to_reference(attn_bias, False)
...
def test_flash_fwd_swa(
    ...
    if flag_gems.vendor_name == "tsingmicro":
        torch_out, torch_lse = attention_ref(
            ref_q,
            ref_k,
            ref_v,
            scale,
...
def attn_bias_from_alibi_slopes(slopes, seqlen_q, seqlen_k, causal=False):
    ...
    if causal:
        return (
            (torch.arange(-seqlen_k + 1, 1, device="cpu", dtype=torch.float32) * (slopes.to("cpu"))).to(device)
        )
...
def ref_paged_attn(
    ...
    for i in range(num_seqs):
        ...
        empty_mask = torch.ones(query_len, kv_len, device="cpu").to(flag_gems.device)
        ...
        if sliding_window is not None:
            ...
            sliding_window_mask = sliding_window_mask.to("cpu")
...
def test_flash_attn_varlen_func(
    ...
    with torch.device(flag_gems.device):
        ...
        if alibi:
            ...
            alibi_slopes = (
                (torch.ones(
                    num_seqs, num_query_heads, device="cpu", dtype=torch.float32
                )
                * 0.3).to(device=device)
            )
            attn_bias = attn_bias_from_alibi_slopes(
        ...
        ref_output = ref_paged_attn(
            ...
            scale=scale,
            attn_bias=attn_bias.to("cpu") if attn_bias is not None else attn_bias,
...
def test_reshape_and_cache(
    ...
    with torch.device(device):
        ...
        # Clone the KV caches.
        cloned_key_cache_ = key_cache.clone()
        cloned_value_cache_ = value_cache.clone()
        cloned_key_cache = cloned_key_cache_.to("cpu")
        cloned_value_cache = cloned_value_cache_.to("cpu")
        ...
        key_cache_ = key_cache.to("cpu")
        value_cache_ = value_cache.to("cpu")
        torch.testing.assert_close(key_cache_, cloned_key_cache)
        torch.testing.assert_close(value_cache_, cloned_value_cache)
...
def test_reshape_and_cache_flash(
    ...
    with torch.device(device):
        ...
        with flag_gems.use_gems():   
            key_cache_ = key_caches[0]
            key_cache = key_cache_.contiguous()  
            value_cache_ = value_caches[0]
            value_cache = value_cache_.contiguous()   
