...
def all_kernel_dim(
    ...
    for off in range(0, N, BLOCK_N):
        ...
        _all = _all and (a != 0)
    all = tl.reduce(_all.to(tl.int8), axis=1, combine_fn=reduce_all)
...
def all_kernel_1(
    ...
    all_val = tl.reduce((inp_val != 0).to(tl.int8), axis=0, combine_fn=reduce_all)
    mid_ptr = mid + pid
...
def all_kernel_2(mid, out, MID_SIZE, BLOCK_MID: tl.constexpr):
    ...
    mid_val = tl.load(mid_ptrs, mask=mask, other=1).to(tl.int8)
