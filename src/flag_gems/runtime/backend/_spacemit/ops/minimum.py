...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("minimum"),
    key=["n_elements"],
)
@triton.jit
def minimum_kernel_tt(
    ...
    for block_idx in tl.range(0, sub_num):
        ...
        if a.dtype == tl.bfloat16:
            a = a.to(tl.float32)
            b = b.to(tl.float32)
        out = tl.minimum(a, b).to(Out_ptr.type.element_ty)
        tl.store(out_blk, out, boundary_check=(0,))


def minimum(A, B):
    logger.debug("GEMS_SPACEMIT MINIMUM")
    A, B = A.contiguous(), B.contiguous()
    out = torch.empty(A.shape, dtype=A.dtype, device=A.device)
    n = A.numel()
    minimum_kernel_tt[(NUM_CTAS,)](A, B, out, n)
    return out
