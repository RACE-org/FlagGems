...
@libentry()
@libtuner(
    configs=runtime.get_tuned_config("maximum"),
    key=["n_elements"],
)
@triton.jit
def maximum_kernel_tt(
    A_ptr,
    B_ptr,
    Out_ptr,
    n_elements,
    ...
    for block_idx in tl.range(0, sub_num):
        ...
        if a.dtype == tl.bfloat16:
            a = a.to(tl.float32)
            b = b.to(tl.float32)
        out = tl.maximum(a, b).to(Out_ptr.type.element_ty)
        tl.store(out_blk, out, boundary_check=(0,))


def maximum(A, B):
    logger.debug("GEMS_SPACEMIT MAXIMUM")
    A, B = A.contiguous(), B.contiguous()
    out = torch.empty(A.shape, dtype=A.dtype, device=A.device)
    n = A.numel()
    maximum_kernel_tt[(NUM_CTAS,)](A, B, out, n)
    return out
