...
def _select_max_block_size(m):
    # Keep per-program footprint small on small-SPM devices.
    if m <= 4096:
        return triton.next_power_of_2(m)
    return 65536


def _max_reduce_staged(inp):
    curr = inp
