...
@pointwise_dynamic(promotion_methods=[(0, 1, "INT_TO_FLOAT")], config=op_config)
@triton.jit
def true_div_func(x, y):
    return x / y


@pointwise_dynamic(is_tensor=[True, False], promotion_methods=[(0, 1, "INT_TO_FLOAT")], config=op_config)
@triton.jit
def true_div_func_tensor_scalar(x, y):
    return x / y


@pointwise_dynamic(is_tensor=[False, True], promotion_methods=[(0, 1, "INT_TO_FLOAT")], config=op_config)
@triton.jit
...
@triton.jit
def _remainder(x, y):
    # For i32/i64/uint32/uint64: use native `x % y`.  The fp32 vector path
    # cannot exactly represent full-range i32 values (fp32 mantissa = 23
    # bits); the int→fp32→int roundtrip produces wrong results when |x| or
    # |y| ≥ 2^24.
    #
    # i16/i8 stay on the fp32 vector path — their entire value range fits in
    # fp32 exactly, and Tx81 int vector ops fall back to slow RISC-V scalar.
    ...
    # Float path (and i16/i8 → fp32): avoids redundant int↔fp32 round-trips.
    x_f = x.to(tl.float32)
    y_f = y.to(tl.float32)
    abs_x = tl.abs(x_f)
    abs_y = tl.abs(y_f)
    # Initial quotient — RECIP may under-estimate by 1.
    q = trunc(abs_x / abs_y)
    # Overshoot correction (mirrors LinalgToMK corrective integer modulo).
    needs_corr = (abs_x - q * abs_y) >= abs_y
    q = tl.where(needs_corr, q + 1.0, q)
    r_abs = abs_x - q * abs_y
    # C-style remainder: sign follows dividend.
    sign_x = x_f < 0.0
    sign_y = y_f < 0.0
    r = tl.where(sign_x, -r_abs, r_abs)
    # Python/torch.remainder convention: sign follows divisor — add y when
    # signs differ and r is non-zero.
    out = tl.where((r_abs != 0.0) & (sign_x ^ sign_y), r + y_f, r)
    return out.to(x.dtype)


@pointwise_dynamic(promotion_methods=[(0, 1, "DEFAULT")], config=op_config)
@triton.jit
def rem_tt(x, y):
    return _remainder(x, y)


@pointwise_dynamic(is_tensor=[True, False], promotion_methods=[(0, 1, "DEFAULT")], config=op_config)
@triton.jit
def rem_ts(x, y):
    return _remainder(x, y)


@pointwise_dynamic(is_tensor=[False, True], promotion_methods=[(0, 1, "DEFAULT")], config=op_config)
@triton.jit
def rem_st(x, y):
    return _remainder(x, y)
