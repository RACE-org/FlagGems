...
def var_mean(x, dim=None, *, correction=None, keepdim=False):
    ...
    if dim is None or len(dim) == x.ndim:
        ...
        acc = torch.empty([BLOCK_NUM], dtype=torch.float32, device=x.device)
        average = torch.empty([BLOCK_NUM], dtype=torch.float32, device=x.device)
        count = torch.empty([BLOCK_NUM], dtype=torch.float32, device=x.device)
