...
from flag_gems.ops.cummin import cummin as _generic_cummin
...
@triton.jit
def _tl_cummin_fidx(input, index, axis=0):
...
@triton.jit
def _tl_min_fidx_tie_break_right(input, index, axis=None, keep_dims=False):
    return tl.reduce(
        (input, index),
        axis,
...
@libentry()
@triton.jit(do_not_specialize=["n_elements"])
def _scan_part_min_fidx_kernel(
    inp,
    out,
    out_indices_f32,
    partial_min,
    partial_min_indices_f32,
    n_elements,
    BLOCK_SIZE: tl.constexpr,
    ...
    vals = tl.load(inp + offs, mask=mask, other=max_value).to(tl.float32)
    ...
    result, cummin_indices = _tl_cummin_fidx(vals, idxs, axis=0)

    if tl.constexpr(NEED_PARTIAL):
        part_min, part_min_indices = _tl_min_fidx_tie_break_right(
            vals, idxs, axis=0
        )

    tl.store(out + offs, result, mask=mask)
    tl.store(out_indices_f32 + offs, cummin_indices, mask=mask)

    if tl.constexpr(NEED_PARTIAL):
        tl.store(partial_min + pid, part_min)
        tl.store(partial_min_indices_f32 + pid, part_min_indices)


@libentry()
@triton.jit(do_not_specialize=["n_elements"])
def _add_base_min_fidx_kernel(
    out,
    out_indices_f32,
    partial_min,
    partial_min_indices_f32,
    ...
    if pid > 0:
        base_val = tl.load(partial_min + pid - 1)
        base_idx = tl.load(partial_min_indices_f32 + pid - 1)

        final_vals = tl.minimum(out_vals, base_val)
        # Current block has larger indices than all previous blocks, so equality
        # keeps the current index and preserves cummin's rightmost tie-break.
...
def _scan_then_fan_col_fidx(inp, out, out_indices_f32, n_ele, dtype, use_out_indices=False):
    ...
    with torch_device_fn.device(inp.device):
        _scan_part_min_fidx_kernel[grid](
            inp,
            out,
            out_indices_f32,
            partial_min,
    ...
    if need_partial:
        ...
        with torch_device_fn.device(inp.device):
            _add_base_min_fidx_kernel[grid](
                out,
                out_indices_f32,
                partial_min,
                partial_min_indices,
...
def _scan_part_min_abc_fidx_kernel(
    inp,
    out,
    out_indices_f32,
    partial_min,
    ...
    max_value = get_dtype_max(inp.type.element_ty)
    vals = tl.load(inp + offset, mask=mask, other=max_value).to(tl.float32)
    if tl.constexpr(USE_OUT_INDICES):
    ...
    tl.store(out_indices_f32 + offset, cummin_indices, mask=mask)

    if tl.constexpr(NEED_PARTIAL):
        tl.store(partial_min + part_offset, part_min)
        tl.store(partial_min_indices_f32 + part_offset, part_min_indices)


@libentry()
@triton.jit(do_not_specialize=["part_num"])
def _add_base_min_abc_fidx_kernel(
    out,
    out_indices_f32,
    partial_min,
    partial_min_indices_f32,
    ...
    if pid_b > 0:
        base_val = tl.load(partial_min + last_part_offset)
        base_idx = tl.load(partial_min_indices_f32 + last_part_offset)
...
def _scan_then_fan_fidx(
    ...
    grid = (A, part_num, C)
    with torch_device_fn.device(inp.device):
        _scan_part_min_abc_fidx_kernel[grid](
            inp,
            out,
            out_indices_f32,
            partial_min,
    ...
    if need_partial:
        ...
        with torch_device_fn.device(inp.device):
            _add_base_min_abc_fidx_kernel[grid](
                out,
                out_indices_f32,
                partial_min,
                partial_min_indices,
...
@libentry()
@triton.jit
def _scan_part_min_abc_loop_fidx_kernel(
    inp,
    out,
    out_indices_f32,
    B,
    ...
    prev_min_idx = tl.full([], 0.0, dtype=tl.float32)
    ...
    for l_idx in tl.range(loop_num):
        ...
        prev_min_idx_b = tl.broadcast_to(prev_min_idx, (BLOCK_SIZE,))

        cummin_indices = tl.where(
            result <= prev_min_val_b, cummin_indices, prev_min_idx_b
        )
        result = tl.minimum(result, prev_min_val_b)

        prev_min_val = tl.sum(tl.where(last_mask, result, 0.0), axis=0)
        prev_min_idx = tl.sum(tl.where(last_mask, cummin_indices, 0.0), axis=0)
...
def _scan_then_fan_loop_fidx(inp, out, out_indices_f32, A, B, C):
    ...
    grid = (A, C)
    with torch_device_fn.device(inp.device):
        _scan_part_min_abc_loop_fidx_kernel[grid](
            inp,
            out,
            out_indices_f32,
            B,
...
def cummin(
    ...
    *,
    ...
    logger.debug("GEMS TSINGMICRO CUMMIN")
    ...
    if input.dtype not in (torch.float16, torch.bfloat16, torch.float32):
        return _generic_cummin(input, dim, out=out)
