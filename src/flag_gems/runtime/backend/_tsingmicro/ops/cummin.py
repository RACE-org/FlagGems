...
def scan_part_min_abc_loop_kernel(
    ...
    # init
    prev_min_val = tl.full([], float("inf"), dtype=tl.float32)
...
def cummin(inp, dim=1, *, dtype=None):
    ...
    if M == 1 and K == 1:
        ...
        # For large M*K, use the loop kernel to avoid tree-reduction that
        # produces memref<1xi1> buffers which crash BoolRelationVVOp in TX81.
        ...
        out_indices.copy_(base_indices)
