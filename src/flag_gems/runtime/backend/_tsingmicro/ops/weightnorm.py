...
class WeightNormInterface(torch.autograd.Function):
    ...
    def forward(ctx, v, g, dim):
        ...
        # Keep ``norm`` in fp32 internally: the backward v_grad formula involves a
        # catastrophic cancellation (w/norm - v/norm**3 * vw_sum), so a norm rounded
        # to fp16/bf16 produces large relative errors in v_grad. The returned norm is
        # cast back to g's dtype to preserve the public output contract.
        norm_fp32  = torch.empty_like(g, dtype=torch.float32)
        if dim == 0:
            M = v.shape[0]
            ...
            with torch_device_fn.device(v.device):
                weight_norm_kernel_first[grid](
                    output, norm_fp32 , v, g, M, N, eps=torch.finfo(torch.float32).tiny
                )
        elif dim == v.ndim - 1:
        ...
        norm = norm_fp32.to(g.dtype)
        ctx.save_for_backward(v, g, norm_fp32)
        ctx.DIM = dim
        return output, norm
