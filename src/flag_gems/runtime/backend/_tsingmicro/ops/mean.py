...
def _select_mean_block_size(m):
    # Favor smaller working sets on small-SPM multi-tile NPUs.
    if m <= 4096:
        return triton.next_power_of_2(m)
    return 65536


def _mean_reduce_staged(inp, dtype):
    total_elems = inp.numel()
    curr = inp
    curr_size = total_elems
    device = inp.device

    with torch_device_fn.device(device):
        while curr_size > 65536:
            block_size = _select_mean_block_size(curr_size)
            mid_size = triton.cdiv(curr_size, block_size)
            mid = torch.empty((mid_size,), dtype=dtype, device=device)
            mean_kernel_1[(mid_size, 1, 1)](curr, mid, curr_size, block_size)
            curr = mid
            curr_size = mid_size

        out = torch.empty([], dtype=dtype, device=device)
        block_mid = triton.next_power_of_2(curr_size)
        mean_kernel_2[(1, 1, 1)](curr, out, total_elems, curr_size, block_mid)
    return out
...
def mean(inp, *, dtype=None):
    ...
    return _mean_reduce_staged(inp, dtype)
