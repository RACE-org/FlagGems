    ...
    with code.indent():
        ...
        with code.indent():
            ...
            for i in range(rank)[::-1]:
                ...
                code.writeline(f"mod = cur_idx.to(tl.int64) % shape_{i}.to(tl.int64)")
                code.writeline(f"mod = mod.to(cur_idx.dtype)")
                ...
                if i != 0:
                    code.writeline(f"cur_idx = (cur_idx.to(tl.int64) // shape_{i}.to(tl.int64)).to(cur_idx.dtype)")
