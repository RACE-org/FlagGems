...
def dropout_heur_block(args):
    # NOTE: BLOCK must stay at 1024. 2048+ produces degenerate philox output
    # (dropout rate collapses to ~0.1% regardless of p), indicating a vector-width
    # limit in the Tx81 philox/uint_to_uniform_float lowering. The rand kernel
    # works around this with a grid=(16,)+inner-loop pattern, but dropout's
    # per-CTA one-shot philox call doesn't fit that pattern without reworking the
    # offset counter.
    n = args["N"]
    if n <= 512:
        return 512
