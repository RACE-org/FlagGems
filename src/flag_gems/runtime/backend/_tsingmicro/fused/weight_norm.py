...
def weight_norm_except_dim_kernel(
    ...
    row_offset = pid + tid_m.to(tl.int32)
    ...
    for base in range(0, v_shape0 * v_shape2, BLOCK_COL_SIZE):
        ...
        n_idx = row_offset.to(tl.int32)
        k_idx = col_offset % v_shape2

        mask = m_idx < v_shape0 and row_mask

        #v_offsets = m_idx * v_shape1 * v_shape2 + n_idx * v_shape2 + k_idx
        nk_idx = n_idx * v_shape2 + k_idx
        v_shape12 = v_shape1 * v_shape2
        v_offsets = m_idx * v_shape12.to(tl.int64) + nk_idx
...
# Backward function will overflow shared memory limit when choose [32, 2048]
def prune_invalid_config(configs, named_args, **kwargs):
    valid_configs = []
    import flag_gems
    ...
    for config in configs:
        if not ((config.kwargs["BLOCK_ROW_SIZE"] == 32) and (config.kwargs["BLOCK_COL_SIZE"] == 2048)):
...
@triton.autotune(
    configs=runtime.get_tuned_config("weight_norm_kernel"),
    key=["v_shape0", "v_shape1", "v_shape2"],
    prune_configs_by={"early_config_prune":prune_invalid_config},
)
@triton.jit(do_not_specialize=["eps"])
